...
#let template(
  ...
  show heading.where(level: 1): it => {
    ...
    let black_rectangle = place(
      dx: -page.margin.outside, dy: -1em, rect(fill: rgb("#9C0017"), width: page.margin.outside - 5pt, height: 2em),
    )
...
#let cover_page(
  title: "", author: "", supervisor: "", co_supervisor: "", department: "DIPARTIMENTO DI INGEGNERIA DELL'INFORMAZIONE", degree: "CORSO DI LAUREA IN INGEGNERIA ...", academic_year: "20XX-20XY", defense_date: "", logo_top_left: "images/unipd-bn.png", logo_top_right: "images/logo-dei.png",
) = {
  // Set up the document
  set document(author: author, title: title)
  set page(
    margin: (left: 2.5cm, right: 2.5cm, top: 2.5cm, bottom: 2.5cm), background: place(
      image("images/unipd-light.jpg", width: 150%, height: 150%, fit: "contain"), dx: -20cm,
    ),
  )

  // Header with logos and department name
  align(left)[
    #box(width: 3cm, image(logo_top_left))
    #h(1fr)
    #box(width: 3cm, image(logo_top_right))
  ]
  ...
  // Thesis title
  align(center)[
    #text(strong(title), size: 18pt)
  ]

  v(4cm)

  // Supervisor and author information
  grid(columns: (1fr, 1fr), column-gutter: 1cm, align(left)[
    Relatore: \
    #strong(supervisor)

    #if co_supervisor != "" [ Correlatore: \
      #strong(co_supervisor) ]
  ], align(left)[
    Laureando: \
    #strong(author)
  ])
  ...
  // Academic year and defense date
  align(center)[
    ANNO ACCADEMICO #academic_year
    #v(0.5cm)

    #if defense_date != "" [
      Data di laurea #defense_date
    ]

  ]
}

#let acknowledgements(content: "", initials: "") = {
...
#let createSection(title: "Title", content: "Content") = {
  heading(title)
  text(content)
}